...
export function parseContent(content) {
  ...
  for (const block of rawBlocks) {
    if (/^\*\*Pregunta \d+:|^\*\*Cuestionario/i.test(block)) {
...
function parseQuiz(block) {
  ...
  for (const line of lines) {
    ...
    if (qMatch) {
      ...
      current = { id: parseInt(qMatch[1]), text: qMatch[2], options: [], correct: null }
      continue
    }
    const aMatch = trimmed.match(/^\*\*Respuesta correcta:\s*[\(]?([a-dA-D])[\).]?/i)
    if (aMatch && current) {
      current.correct = aMatch[1].toLowerCase()
      continue
    }
    if (current && /^[a-dA-D][\)\.]\s/.test(trimmed)) {
