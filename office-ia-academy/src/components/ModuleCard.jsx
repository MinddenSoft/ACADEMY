...
export default function ModuleCard({ module }) {
  ...
  return (
    <div className={`rounded-2xl border transition-all duration-300 ${isOpen ? 'bg-white dark:bg-dark-800 border-cyan-500/20 shadow-lg shadow-cyan-500/5 dark:shadow-none' : 'bg-white dark:bg-dark-800/50 border-slate-200 dark:border-white/5 hover:border-slate-300 dark:hover:border-white/10 shadow-sm dark:shadow-none'}`}>
      <button
      ...
      >
        ...
        <div className={`flex-shrink-0 ml-4 w-10 h-10 rounded-xl bg-slate-100 dark:bg-white/5 flex items-center justify-center transition-all duration-300 ${isOpen ? 'bg-cyan-500/20 dark:bg-cyan-500/20 rotate-180' : ''}`}>
      ...
      <div className={`accordion-content ${isOpen ? 'open' : ''}`}>
        <div className="px-6 lg:px-8 pb-6 lg:pb-8 pt-0">
          <p className="ml-16 text-slate-600 dark:text-slate-400 text-sm mb-6 leading-relaxed">
            <span className="text-cyan-500 dark:text-cyan-400 font-semibold">Objetivo:</span> {module.objective}
          </p>
                ...
                <ul className="space-y-2 ml-4">
                  {section.content.split('\n').filter((p) => p.trim() && !p.startsWith('#') && !p.startsWith('**')).slice(0, 3).map((item, ii) => (
                    <li key={ii} className="text-slate-600 dark:text-slate-400 text-sm leading-relaxed flex items-start gap-2">
                      <span className="text-cyan-500/60 dark:text-cyan-500/40 mt-1.5 flex-shrink-0">
          ...
          <div className="ml-16 mt-8">
            <Link
              ...
              className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-gradient-to-r from-cyan-500 to-violet-500 text-white font-semibold text-sm hover:shadow-lg hover:shadow-cyan-500/25 transition-all duration-300 hover:-translate-y-0.5"
            >
