...
import { parseContent, renderItem } from '../utils/renderInteractiveContent'
...
export default function SectionDetailPage() {
  ...
  const exerciseBlocks = useMemo(() => {
    const exerciseSections = sections.filter(s =>
      /ejercicio/i.test(s.heading) && !/^ejercicios\s/i.test(s.heading)
    )
    ...
    <div className="min-h-screen bg-neutral-50 dark:bg-dark-900 relative">
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 pt-24 pb-10">
        ...
        <Link
          ...
          className="inline-flex items-center gap-2 text-neutral-400 dark:text-neutral-400 hover:text-white dark:hover:text-white transition-colors mb-5 text-sm font-medium"
        >
...
function ExerciseCard({ ex, index, cfg }) {
  ...
  return (
    <div className="bg-neutral-50 dark:bg-dark-800/50 border border-neutral-100 dark:border-white/5 rounded-xl p-5">
      ...
      <div className="bg-white dark:bg-dark-900 border border-neutral-200 dark:border-white/10 rounded-lg px-4 py-3.5 font-mono text-sm text-neutral-600 dark:text-neutral-300 leading-relaxed mb-3 break-words">
        {ex.text}
      </div>
      <button
        onClick={handleCopy}
        className={`inline-flex items-center gap-1.5 text-sm px-4 py-1.5 rounded-lg font-medium transition-all ${
